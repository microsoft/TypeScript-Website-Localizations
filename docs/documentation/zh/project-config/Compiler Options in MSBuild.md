...
当你有一个基于 MSBuild 的项目，其使用了 TypeScript，就比如一个 ASP.NET Core 的项目，你可以通过两种方式配置 TypeScript。通过 `tsconfig.json` 或者通过该项目设置设置项。
...
我们建议尽可能地在您的项目使用 `tsconfig.json`。在现代版本的 Visual Studio 中，有一项名为「TypeScript JSON 配置文件」的选项元素，可以向现有项目中添加一个配置。
